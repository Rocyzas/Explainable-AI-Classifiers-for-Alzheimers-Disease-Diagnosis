...
def logClassifier(clf, classes, accuracy, recall, precision, f1, roc_auc, report, matrix, parameters):
    ...
    if os.path.exists(modelsLog)!=True:
        with open(modelsLog, mode='w') as f:
            ...
            writer.writerow(["Classifier", "Classification Groups",
                            ...
                            "ROC_AUC", "Report", "Confusion Matrix", "HyperParameters", "Time"])
            f.close()
    ...
    with open(modelsLog, mode='a+') as file_object:
        ...
        writer.writerow([clf.__class__.__name__, classes, accuracy, recall, precision, f1,
                        roc_auc, report, matrix, parameters, time])
