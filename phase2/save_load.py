...
def logClassifier(clf, iProj, accuracy, recall, precision, f1, AUC, report, matrix):
    ...
    modelsLog = "/media/rokas/HDD/Phase2/Metrics/log.csv"
...
def plotter(history, clf=None, iProj=None):
    ...
    if type(history) is list:
        ...
        path = uniquify('/media/rokas/HDD/Phase2/Metrics/' + clf + iProj + '_ACC.png')
        plt.savefig(path)
        # plt.show()
    ...
    # plt.plot(loss)
    # plt.plot(loss_val)
    # plt.title(clf + ' loss on ' + iProj)
    # plt.ylabel('loss')
    # plt.xlabel('epoch')
    # plt.legend(['train', 'validation'], loc='upper left')
    # path = uniquify('/media/rokas/HDD/Phase2/Metrics/' + clf + iProj + '_LOSS.png')
    # plt.savefig(path)
    # plt.show()
